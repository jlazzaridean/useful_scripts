...
def load():
    ...
    # This part is lifetime-specific; assume we have a TIFF stack where
    # first channel is photons (could be stored as int) and second
    # channel is lifetime (currently in seconds as a float). We want to
    # clean up the data such that it can be safely cast to int.
    data[:, :, 1, :, :] = data[:, :, 1, :, :] * 1e12 # convert to picoseconds
    np.around(data, decimals=0, out=data)
...
viewer.layers.selection.add(viewer.layers['Human labels'])
